"""Import 'Disposed Cases.csv' from Karpel"""
...
# --- Define import_fld function --- 
def import_disp(disp_df_path: str, export_path: str):

    # Initialize Path object
    import_path = Path(disp_df_path)

    # Initialize 'Corrected Columns Headers' list from helper_files
    col_df = pd.read_excel(Path("helper_files/Corrected Columns Headers.xlsx"), sheet_name="Disp")
    ...
    # Date columns 
    disp_date_cols = get_date_cols(keep_names)

    # Create converters dict
    # Create a dictionary of converters for specific columns
    str_columns = {k: str_cleaner(uppercase=False) if k == 'charge_description' else str_cleaner() 
                    for k, v in col_dtypes.items() if v == 'str' or k == 'charge_description'}
    ...
    for col_name in disp_date_cols:
    ...
    charges = df[[
        ...
        'offense_state', 
        'offense_zipcode',
        'disp_code', # -- don't assume case-unique 
        'disp_date' # -- don't assume case-unique
    ]].copy()
    ...
    file_court_num = df[[ # should tell us which referred cases have been filed 
        'pbk_num',
        'court_num' # -- all should have a court number, since a case can only be disposed if it's been filed
    ]].copy()
    ...
    file_report_num = expand_id_pairs(file_report_num, col1='pbk_num', col2='report_num')

    # Export as CSV 
    export_df(df, "disp_df", export_path)
    export_df(cases, "disp_cases", export_path)
    export_df(charges, "disp_charges", export_path)
    export_df(file_court_num, "disp_file_court_num", export_path)
    export_df(file_report_num, "disp_file_report_num", export_path)
