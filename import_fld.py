"""Import 'Filed Cases.csv' from Karpel"""
...
def import_fld(fld_df_path: str, export_path: str):

    # Initialize Path object
    import_path = Path(fld_df_path)

    # Initialize 'Corrected Columns Headers' list from helper_files
    col_df = pd.read_excel(Path("helper_files/Corrected Columns Headers.xlsx"), sheet_name="Fld")
    ...
    # Date columns 
    fld_date_cols = get_date_cols(keep_names)
    ...
    # Export as CSV 
    export_df(df, "fld_df", export_path)
    export_df(cases, "fld_cases", export_path)
    export_df(charges, "fld_charges", export_path)
    export_df(file_court_num, "fld_file_court_num", export_path)
    export_df(file_report_num, "fld_file_report_num", export_path)
